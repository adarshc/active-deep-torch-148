...
explored_samples_added_per_update = total_classes
exploited_samples_added_per_update = total_classes
...
function train()
   ...
   local correct_predictions = 0
    ...
    if active_learning_flag then
      ...
      if exploration_flag then
        print('[Exploring...]')

        kernel_distance_measures, unlabeled_training_indices = calculateKernelDistance()
        explored_samples_added_per_update = math.min(explored_samples_added_per_update,(training_threshold - current_pool_size))
        top_k_values, top_k_indices = kernel_distance_measures:topk(explored_samples_added_per_update, true)
        ...
        exploited_samples_added_per_update = math.min(exploited_samples_added_per_update,(training_threshold - current_pool_size))
...
while true do
  if initial_training_counter > 0 then
    ...
    --  epoch_per_update = 5
    --end
    print('[training using exploration-exploitation method...]')
    train()
    print('[testing using exploration-exploitation method...]')
    test()
  end
end
